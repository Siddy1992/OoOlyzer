#!/usr/bin/env python3
"""
OoOLyzer - unified leakage detector for gem5 O3 RISC-V traces.

Two detectors in one tool, selected with --mode:

  --mode ril   (DEFAULT)  Rename-Induced Transition Leakage  [unchanged behaviour]
       Consecutive committed writes to the SAME physical register by DIFFERENT
       arch registers (shares) -> Hamming-distance transition. This is the
       PRESENT key-recovery channel (Sec. 4.3-4.7).

  --mode iew              IEW-Induced Dispatch Leakage  (Sec. 4.8 / App. A.4)
       Marker-instrumented PINI/HPC2 gadget. For each configured pair of markers
       it compares program order (Commit) vs execution order (Exec): reports
       execution-order INVERSION ("final executed BEFORE contributor") and
       [execute,commit] window CO-RESIDENCY. This is the half the RIL detector
       does not cover (it parses the Commit stream the RIL side never needs).

  --mode both             Run both detectors on the same trace (two passes).

BACKWARD COMPATIBILITY
  Default mode is 'ril'. All previous flags, output, and the RIL selftest are
  unchanged. --selftest now runs BOTH the RIL and IEW selftests.

REQUIRED gem5 FLAGS
  RIL : --debug-flags=Rename,Exec,ExecFetchSeq,FreeList,IEW,Commit,IQ,LSQ,Scoreboard
  IEW : --debug-flags=Exec,ExecFetchSeq,Commit          (ExecFetchSeq + Commit essential)

CONFIG (one JSON may carry both schemas; keys are read per mode)
  RIL : { "arch_map": {"20":"a0","29":"a1"}, "conflicts": [["a0","a1"]] }
  IEW : { "markers": {"z13":19,"z31":49,...}, "pairs": [["z13","z31"],...], "delta":1 }

USAGE
  ooolyzer.py --trace t.out [--config c.json] [--json o.json]               # RIL
  ooolyzer.py --mode iew  --trace t.out --config pini_iew_d3.json [--delta N]
  ooolyzer.py --mode both --trace t.out --config combined.json
  ooolyzer.py --selftest
"""
...
# --- shared Exec / sequence-number regexes (used by BOTH detectors) ---------
...
# ===========================================================================
# RIL DETECTOR  (verbatim from the original ooolyzer.py)
# ===========================================================================
...
def report_text(cands, pairs, stats, mode, top=20):
    ...
    print("=" * 74)
    print(f"OoOLyzer report   [enforcement: {mode}]")
    print("=" * 74)
    ...
    if stats["rename_dest"] == 0 or stats["joined"] == 0:
        print("  !! a required stream is empty -- check regex / flags.")
...
def report_iew(cands, stats, name2id):
    print("=" * 74)
    print("OoOLyzer IEW report   (IEW-Induced Dispatch Leakage, PINI experiment)")
    print("=" * 74)
    ...
    if stats["marker_hits"] == 0:
        print("  !! no markers found -- check MARK-instrumented gadget and "
              "--debug-flags=Exec,ExecFetchSeq.")
    if stats["commit_lines"] == 0:
        print("  !! no commit lines -- add Commit to --debug-flags.")
    ...
    print(f"  co-residency overlaps      : {len(cor)}   (App. A.4 dispatch-leakage signal)")
    print()
    for c in cands:
        flags = []
...
def main():
    ap = argparse.ArgumentParser(description="OoOLyzer unified RIL + IEW detector")
    ap.add_argument("--mode", choices=["ril", "iew", "both"], default="ril",
                    help="ril (default, unchanged), iew (Sec 4.8), or both")
    ap.add_argument("--trace")
    ap.add_argument("--config")
    ap.add_argument("--json")
    # RIL options
    ap.add_argument("--loose", action="store_true",
                    help="[ril] allow squashed allocations between two committed writes")
    ...
    ap.add_argument("--delta", type=int, default=1,
                    help="[iew] min [exec,commit] overlap (ticks) for co-residency")
    ...
    out = {}
    if a.mode in ("ril", "both"):
        j = run_ril(a, cfg)
        if j is not None:
            out["ril"] = j
    if a.mode in ("iew", "both"):
        if a.mode == "both":
            print()
        out["iew"] = run_iew(a, cfg)

    if a.json and out:
        payload = out if a.mode == "both" else next(iter(out.values()))
