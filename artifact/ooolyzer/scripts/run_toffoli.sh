...
CFG=$ARTIFACT_ROOT/configs/toffoli.json
G5CFG=$ARTIFACT_ROOT/configs/gem5_present_toffoli.py
ELF=${TOFFOLI_ELF:-$ARTIFACT_ROOT/bin/toffoli32.elf}
if [ "${REBUILD:-0}" = 1 ]; then
    ELF=$ARTIFACT_ROOT/build/toffoli32.elf
    "$ARTIFACT_ROOT/scripts/build_toffoli.sh" "$ELF"
fi
...
OD=$ARTIFACT_ROOT/m5out/toffoli/o3
RID=$ARTIFACT_ROOT/results/toffoli
rm -rf "$OD"
mkdir -p "$OD" "$RID"
...
"$PYTHON" "$TOOL" --trace "$OD/trace.out" --config "$CFG" \
  --json "$RID/toffoli_ril.json" --top 20
"$PYTHON" "$TOOL" --trace "$OD/trace.out" --config "$CFG" --filter-conflicts \
  --json "$RID/toffoli_conflicts.json" --top 20

IOD=$ARTIFACT_ROOT/m5out/toffoli/inorder
rm -rf "$IOD"
mkdir -p "$IOD"
...
"$PYTHON" "$TOOL" --trace "$IOD/trace.out" --config "$CFG" --filter-conflicts \
  --json "$RID/toffoli_inorder.json" --top 10 || true
"$PYTHON" "$ARTIFACT_ROOT/scripts/verify_results.py" --case toffoli
